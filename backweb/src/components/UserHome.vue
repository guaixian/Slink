...
const totalImages = 426
const usedStorage = '24.90 MB'
const totalStorage = '4.88 GB'
const storageUsage = 0.5 // 使用百分比
...
const handleDrop = (e: DragEvent) => {
  ...
  if (files && files.length > 0) {
    console.log(`已拖拽 ${files.length} 个文件`)
    // 这里可以添加实际的上传逻辑
  }
}
...
const handleFileSelect = (event: Event) => {
  ...
  if (target.files && target.files.length > 0) {
    console.log(`已选择 ${target.files.length} 个文件`)
    // 这里可以添加实际的上传逻辑
  }
}
